    ...
    def is_required_columns_exist(self, dataframe: DataFrame):
        try:
            columns = list(filter(lambda x: x in self.schema.required_columns,
                                  dataframe.columns))
            logger.info(f"1 , Columns_Count in is_required function: {len(columns)}")
            logger.info(f"2, Columns_Count in is_required function: {len(self.schema.required_columns)}")
